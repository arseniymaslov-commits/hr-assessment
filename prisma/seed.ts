...
async function main() {
  ...
  const assessmentCriteria = [
    [
      "Сроки исполнения и обратной связи",
      "Оценивается соблюдение сроков по запросам, письмам, заявкам, согласованиям, поручениям и предоставлению обратной связи."
    ],
    [
      "Качество предоставленных данных и документов",
      "Оценивается полнота, корректность и достоверность предоставленных данных, документов, таблиц, отчётов и пояснений."
    ],
    [
      "Качество рабочего взаимодействия",
      "Оценивается исполнение совместных задач без необоснованных задержек, повторных запросов, формального подхода и некорректной коммуникации."
    ],
    [
      "Исполнение аудиторских рекомендаций",
      "Оценивается своевременность и качество исполнения рекомендаций по итогам аудиторских проверок."
    ],
    [
      "Взаимодействие при проверках",
      "Оценивается своевременность предоставления документов, пояснений и данных по запросам СВКА."
    ],
    [
      "Трудовая дисциплина",
      "Оцениваются опоздания, отсутствия, нарушения графика работы, некорректная регистрация в СКУД, неоформленные рабочие выезды и несвоевременное информирование УЧР."
    ],
    [
      "Кадровое и административное делопроизводство",
      "Оценивается своевременность и корректность оформления приёма, увольнения, отпусков, табелей, КПЭ, актов передачи дел и иных кадровых документов."
    ],
    [
      "Оценка, обучение и кадровая работа",
      "Оценивается участие сотрудников в оценке, обучении, срезах знаний, формировании кадрового резерва, а также взаимодействие при подборе и предоставление обратной связи."
    ]
  ] as const;

  for (const [name, description] of assessmentCriteria) {
    await prisma.criterion.upsert({
      where: { name },
      update: { description, isActive: true },
      create: { name, description, weight: 1, isActive: true }
    });
  }
